...
export async function saveTokenFromCode(code, redirectUri) {
  ...
  try {
    ...
    const nickname = userData.nickname || `SELLER_${data.user_id}`;

    // 3. Salva no banco com nickname
    ...
    console.log("[Token] Salvo com nickname:", nickname);
    return token;
  } catch (error) {
